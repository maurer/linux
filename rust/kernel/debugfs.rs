...
impl Dir {
    ...
    >(
        ...
        _f: &'static F,
        _w: &'static W,
    ) -> impl PinInit<File<T>, E> + use<'_, 'b, T, TI, E, F, W> {
        let file_ops =
    ...
    >(
        ...
        data: TI,
        _w: &'static W,
    ) -> impl PinInit<File<T>, E> + use<'_, 'b, T, TI, E, W> {
    ...
    >(
        ...
        name: &'a CStr,
        init: F,
    ) -> impl PinInit<Scope<T>, E> + use<'_, 'a, T, E, TI, F> {
        Scope::new(data, |data| {
...
impl<T> Scope<T> {
    ...
    fn new<E, TI: PinInit<T, E>, F: for<'a> FnOnce(&'a T) -> Entry<'static>>(
        data: TI,
        init: F,
    ) -> impl PinInit<Self, E> {
        try_pin_init! {
...
impl<T> Scope<T> {
    ...
    >(
        ...
        name: &CStr,
        init: F,
    ) -> impl PinInit<Self, E> + use<'_, T, E, TI, F> {
        Scope::new(data, |data| {
